...
public class CameraHandler3 : MonoBehaviour
{
[Header("Target")]
...
private float zoomDistance = 7.0f;
...
// Récupère la position actuelle de la souris en pixels sur l'axe des X
float mouseX = Input.mousePosition.x;

Debug.Log(mouseX);  

// Vérifie si la position de la souris est supérieure à 1.2 sur l'axe des X
if (mouseX <= 480)
{
            // Si la condition est vraie, affiche un message dans la console
            Debug.Log("La souris est dans la zone de menu ");
    
    // Termine la fonction pour éviter que le code suivant ne s'exécute
    return;
}
...
Vector3 camPosition = productObj.position + directionalVector *
zoomDistance;
